...
node {
   ...
   // Hence you need to pass 2 or more depending on how many RTC checkouts have been called after @Library directive.
   def snapshotUUID = rtcutils.getSnapshotUUID(2)
   echo "${snapshotUUID}"
}
----

If you don't have Global Shared Library defined in your environment, consult https://jenkins.io/doc/book/pipeline/shared-libraries/[Extending with Shared Libraries] for creating and accessing a shared library in your pipeline script.Note that if you use RTC for hosting the Global Shared Library, then
...
. Under Steps, click the dropdown next to Sample Step and select rtcBuild.
. Select a Build toolkit. 
. Enter a value for RTC server URI or accept the default.
. Select the appropriate credentials or accept the default.
. In the dropdown next to Task, select "Request Build".
. Enter a Build Definition ID.
. Click Generate Pipeline Script.

The following snippet prints the build result UUID after rtcBuild step completes requesting a build in RTC.

[source,syntaxhighlighter-pre]
----
  def ret = rtcBuild buildTool: '<builtoolkit-name>', credentialsId: '<credentials>', serverURI: '<rtc-server-uri>', task: [buildDefinitionId: '<build-definition-id>', name: 'requestBuild'], timeout: 480
...
* The RTC build has reached one of the desired states.
...
* The status of the build - one of OK, ERROR, WARNING, INFO.
* The state of the build - one of COMPLETE, INCOMPLETE, IN_PROGRESS, CANCELED, NOT_STARTED.
...
. Enter a value for RTC server URI or accept the default.
. Select the appropriate credentials or accept the default.
. In the dropdown next to Task, select "Wait for Build".
...
In the following snippet, the "Wait for Build" task uses the build result UUID from "Reqeust Build" task to wait for the build result to enter into COMPLETED or INCOMPLETE state for 120 seconds. Once the step completes, it prints the following fields from the return value 
...
----
  ...
  // Note that  ${ret1.buildResultUUID} is enclosed in double quotes.
  ...
  echo "${ret1.buildState}"
  echo "${ret1.buildStatus}"
  echo "${ret1.timedout}"
----
...
NOTE: The default behavior of creating "Related artifact" link to a Jenkins build in all the accepteed work items when using Repository Workspace or Stream j
...
* In this release, we have changed the behavior of creating "Related artifact" links to Jenkins builds in all the accepted work items originally introduced b
** See Work Item 461859: Make the "creation of Jenkins build links to work items in accepted change sets" an opt - in for the users in Repository Workspace a
