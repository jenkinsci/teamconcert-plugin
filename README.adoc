...
====List Logs
Use the "List Logs" task to retrieve the details of the log files from an EWM build result. Log files appear in the "Logs" tab of the EWM build result. 

NOTE: The content of the log file can be present in the EWM repository or in some external store. This task will  retrieve the details of the log files whose content is stored in the EWM repository. Such logs files can be uploaded to the build result using the logPublisher ant task in the EWM build toolkit.  https://www.ibm.com/docs/en/elm/7.0.2?topic=contributions-logpublisher[Log Publisher]
...
====List Artifacts
Use the "List Artifacts" task to retrieve the details of the log files from an EWM build result. Log files appear in the "Downloads" tab of the EWM build result. 
