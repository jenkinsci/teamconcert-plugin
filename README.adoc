...
NOTE: The content of the log can be present in the EWM repository or in some external store. This task will  download the content of  a log which is stored  in the EWM repository. Such logs can be uploaded to the build result using the logPublisher ant task in the EWM build toolkit. This task is similar to the logRetriever ant task in the EWM build toolkit.  Refer to https://www.ibm.com/docs/en/elm/7.0.2?topic=contributions-logpublisher[Log Publisher] and https://www.ibm.com/docs/en/elm/7.0.2?topic=contributions-logretriever[Log Retriever].
